...
class OptionsPremiumCalculator {
    ...
    async checkKiteLoginStatus() {
        ...
        try {
            const status = await this.kiteAPI.checkLoginStatus();
            this.showAlert(`✅ ${status.message}`, 'success');
            if (dataModeElement) {
                dataModeElement.textContent = `🟢 Live data: ${status.message}`;
                dataModeElement.style.color = '#28a745';
            }
        } catch (error) {
            console.log('API connection failed:', error.message);
            
            if (dataModeElement) {
                dataModeElement.innerHTML = `🔴 Demo mode: ${error.message} • <button onclick="location.reload()" style="background: #007bff; color: white; border: none; padding: 5px 10px; border-radius: 3px; cursor: pointer; margin-left: 10px;">Setup API</button>`;
                dataModeElement.style.color = '#dc3545';
            }
            
            this.showAlert('Using demo mode with simulated data. Click "Setup API" for live data.', 'info');
        }
    }
}
